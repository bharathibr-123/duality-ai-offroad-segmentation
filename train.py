"""
train.py — Deep Thinkers | Offroad Semantic Segmentation
Hackathon-optimized training script with:
  - DeepLabV3+ with EfficientNet-B4 backbone (stronger than MobileNetV2)
  - Combined Focal + Dice loss for rare class handling
  - Advanced augmentation pipeline
  - Mixed precision training (GPU) / CPU fallback
...
IMAGE_SIZE  = 384          # Larger input = better detail
BATCH_SIZE  = 2
NUM_EPOCHS  = 40
LR          = 3e-4
NUM_WORKERS = 0
...
print(f"Device: {DEVICE}  |  AMP: {USE_AMP}  |  Image size: {IMAGE_SIZE}")
...
# ─────────────────────────────────────────
#  MODEL — EfficientNet-B4 backbone (stronger)
# ─────────────────────────────────────────
...
print(f"Model: DeepLabV3+ / EfficientNet-B4  |  Params: {sum(p.numel() for p in model.parameters()):,}")
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # One-hot encode targets, ignoring ignore_index
...
class CombinedLoss(nn.Module):
    def __init__(self, weight=None, focal_gamma=2.0, dice_w=0.4):
        super().__init__()
    ...
    def forward(self, inputs, targets):
        return (1 - self.dice_w) * self.focal(inputs, targets) + \
               self.dice_w * self.dice(inputs, targets)


criterion = CombinedLoss(weight=CLASS_WEIGHTS, focal_gamma=2.0, dice_w=0.4)
...
for epoch in range(NUM_EPOCHS):
    ...
    # Per-class IoU at every 5th epoch
    if (epoch + 1) % 5 == 0:
